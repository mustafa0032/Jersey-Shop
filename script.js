...
// ── Checkout form — sends via EmailJS ────────────────────────────────────────
...
if (checkoutForm) {
  ...
  checkoutForm.addEventListener("submit", async (event) => {
    ...
    // ── Save order locally IMMEDIATELY (before EmailJS, always runs) ──
    ...
    fetch("/api/order/submit", {
      ...
      body: JSON.stringify({
        order_id,
        customer: { name: fullName, email, phone, address, city, postalCode, country },
        items:    orderItems,
    ...
    // Shared template params used by both sends
...
PAYMENT DETAILS
----------------------------
Please transfer CHF ${total.toFixed(2)} using one of the options below:

TWINT: +41 76 412 39 02
IBAN:  CH61 0878 1000 2041 3350 0

IMPORTANT: Use your Order ID as the payment reference:
>> ${order_id} <<

Once we receive your payment, we will contact you to confirm your order. Thank you!
----------------------------`,
    };
    ...
    try {
      ...
      errorMessage.textContent = "⚠️ Could not send the order. Please try again or contact us directly.";
      errorMessage.hidden = false;
    } finally {
